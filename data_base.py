...
def salvar_para_arquivo(biblioteca, categorias):
    f= open('biblioteca.txt', 'a')
    f.write("nome|autor|categoria|preco\n")
    for livro, informacao_livro in biblioteca.items():
            g.write(f"{livro}|{informacao_livro['autor']}|{informacao_livro['categoria']}|{informacao_livro['preço']}\n")

    g= open('categorias.txt', 'a')
    for categoria, livros in categorias.items():
              g.write(f"{categoria}|{','.join(livros)}\n")
    f.close()
def carregar_do_arquivo():
    ...
    with open('biblioteca.txt', 'r', encoding="utf8") as f:
        next(f)  # Ignora a primeira linha (cabeçalho)
        for line in f:
    ...
    with open('categorias.txt', 'r', encoding="utf8") as f:
        for line in f:
            categoria, livros_str = line.strip().split('|')
            livros = livros_str.split(',')
            categorias[categoria] = livros

    return biblioteca, categorias

dinheiro_livro = 0
biblioteca, categorias = carregar_do_arquivo()
...
while True:#inicamos o código com o while true para rodar enquanto Nathália não pedir para parar.
    ...
    if escolha == '1':#esse if serve para caso ela escolha a primeira opção o código rodar de maneira correta.
        nome_livro = input("Digite o nome do livro: ")
        autor_livro = input("Digite o autor do livro: ")
        categoria_livro = input("Digite a categoria do livro: ")
        while True:
            try:#aqui temos um tratamento de erro utilizamos o comando try para caso ocorra um erro possamos escolher o que vai aparecer na tela.
                preco_livro = float(input("Digite o preço do livro: "))
                break  # Se o número for válido, saia do loop
            except ValueError:#O comando "except ValueError" serve para caso ocorra algum erro a mensagem selecionada ira aparecer na tela.
                print("Preço inválido. Tente novamente.")
        informacao_livro = {
            "autor": autor_livro,
    ...
    elif escolha == '2':#O comando "elif,foi utilizado para caso Nathália escolha a opção 2.
        os.system('cls')
